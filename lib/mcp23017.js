...
Mcp23017.prototype.scan = function (cb) {
    Cylon.Logger.info('scanning i2c bus for MCP23017 at 0x20..0x27');
    for(var ii = 0x20; ii < 0x28; ii++){
        Cylon.Logger.info("scan " + ii + "...");
        try{
            this.connection.i2cRead(ii, 0x00, 1, function (err, data) {
                Cylon.Logger.info("scan cb...");
                if(cb != null){
                    cb(err, ii, data);
                }
            });
        } catch(ex){
            Cylon.Logger.info("Exception: " + ex);
        }
    }
};
